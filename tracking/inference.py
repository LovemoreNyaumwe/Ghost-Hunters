...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # check if the ghost has been captured by pacman
        if noisyDistance == None:
            # make every value in the new particle list just the jail position
            newParticles = []
            for i in range(len(self.particles)):
                newParticles.append(self.getJailPosition())
            self.particles = newParticles
        # if the ghost hasn't been captured
        else:
    ...
    def resample(self, oldParticles, counter):
        # just make a new list and repopulate it with particles generated by util.sample(counter), counter containing the set of legal positions.
        # Made this its own function because i was making it way too complicated at first.
        newParticles = []
        for i in range(self.numParticles):
            newParticles.append(util.sample(counter))
        return newParticles
